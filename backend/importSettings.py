...
set_name_url = f"http://{ip}/usapi?method=set-name&name=westonWasHere"
export_settings_url = f"http://{ip}/usapi?method=export-settings&file-name=mySettings.json"

# Create a session that will persist cookies
with requests.Session() as session:
    try:
        # Log in to receive the sid cookie
        login_response = session.get(login_url)
        login_response.raise_for_status()

        if login_response.status_code == 200:
            # Optionally, process the returned JSON if needed
            login_data = login_response.json()
            print("Login successful. Received JSON:", login_data)
        else:
  ...
  "rec-channels": [
    {"id": 0, "type": 0, "is-use": 1, "stream-index": 0, "mode": 0, "dir-name": "AIO_09_REC_Folder", "file-prefix": 0, "prefix-name": "AIO_09_", "file-suffix": 0, "time-unit": 10, "audio": 0},
    {"id": 1, "type": 1, "is-use": 0, "stream-index": 0, "mode": 0, "dir-name": "AIO-09", "file-prefix": 0, "prefix-name": "AIO-09", "file-suffix": 0, "time-unit": 90, "audio": 0},
    {"id": 2, "type": 2, "is-use": 0, "stream-index": 0, "mode": 0, "dir-name": "REC_Folder", "file-prefix": 0, "prefix-name": "VID", "file-suffix": 0, "time-unit": 30, "audio": 0}
  ],
  "nas": [],
  "send-file-cloud": [],
  "schedulers": [],
  ...
  "lcd-control": {"no-touch": 0, "page-idx": 1, "no-flip": 0, "duration": 666667}
    ...
    result = import_settings(session, ip, modified_settings)
    print(result)
