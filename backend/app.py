...
origins = [
    ...
    # Add any other origins you plan to support.
]
...
def get_modified_settings(magewell_id: str) -> dict:
    """Return the settings dictionary with Magewell ID injected where needed."""
    return {
        "is-low-latency": 0,
        "is-auto-send-file": 0,
        "is-auto-del-file": 0,
...
async def run_bulk_update(devices: list):
    max_concurrent = 10  # Limit concurrent device processes.
...
async def bulk_update(file: UploadFile = File(...)):
    """
    Upload a CSV file containing device information (columns: 'Magewell ID' and 'Magewell IP').
    This endpoint starts the bulk update process in the background.
    """
    content = await file.read()
    # Parse CSV from in-memory file
    csv_file = io.StringIO(content.decode("utf-8"))
    reader = csv.DictReader(csv_file)
    devices = [row for row in reader]

    # Start the bulk update process as a background task.
    # We use asyncio.create_task to schedule it without blocking the response.
